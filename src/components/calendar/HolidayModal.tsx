...
export function HolidayModal({ holiday, isOpen, onClose, locale = 'id' }: HolidayModalProps) {
  if (!holiday) return null

  const getTypeLabel = (type: Holiday['type']) => {
    if (locale === 'id') {
      switch (type) {
        case 'national': return 'Hari Libur Nasional'
        case 'religious': return 'Hari Libur Keagamaan'
        case 'regional': return 'Hari Libur Daerah'
  ...
  const getReligionLabel = (religion: Holiday['religion']) => {
    ...
    if (locale === 'id') {
      switch (religion) {
        ...
        case 'christian': return 'Kristen'
        case 'catholic': return 'Katolik'
        case 'hindu': return 'Hindu'
        case 'buddhist': return 'Buddha'
        case 'confucian': return 'Konghucu'
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 pt-2 border-t">
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {holiday.isVariable 
                ? (locale === 'id' ? 'Tanggal berubah setiap tahun' : 'Date varies yearly')
                : (locale === 'id' ? 'Tanggal tetap setiap tahun' : 'Fixed date yearly')
              }
            </div>
            
            {holiday.source && (
              <div className="text-xs text-muted-foreground">
                {locale === 'id' ? 'Sumber: ' : 'Source: '}{holiday.source}
              </div>
            )}
          </div>
        </div>
