...
export function TodayInfo({ holidays, locale = 'id' }: TodayInfoProps) {
  ...
  const isDayOff = isWeekend || todayHoliday?.type === 'national' || todayHoliday?.type === 'joint_leave'
            ...
            <div className="text-right">
              <div className="text-sm font-medium text-card-foreground">
                {isDayOff 
                  ...
                  : (locale === 'id' ? 'Hari Kerja' : 'Working Day')
                }
              </div>
              {todayHoliday ? (
                <div className="flex items-center gap-1 mt-1">
                  <Badge variant={getTypeVariant(todayHoliday.type)} className="text-xs">
