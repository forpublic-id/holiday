...
export type HolidayFilterType = 'all' | 'national_joint' | 'regional' | 'national' | 'joint_leave'
  ...
  const filterOptions = [
    {
      ...
      description: locale === 'id' ? 'Libur nasional dan cuti bersama' : 'National holidays and joint leave'
    },
    {
      value: 'regional' as HolidayFilterType,
      label: locale === 'id' ? 'Libur Daerah' : 'Regional Holidays',
      count: holidayCounts.regional,
      description: locale === 'id' ? 'Libur khusus daerah' : 'Regional holidays'
    },
    {
...
export function filterHolidaysByType(holidays: Holiday[], filterType: HolidayFilterType): Holiday[] {
  switch (filterType) {
    case 'national_joint':
      return holidays.filter(h => h.type === 'national' || h.type === 'joint_leave')
    case 'regional':
