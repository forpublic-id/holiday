import { Holiday, HolidayFilter, HolidaySearchResult, LongWeekend, Province } from '@/types/holiday'
 ...
 */
export function filterHolidays(holidays: Holiday[], filter: HolidayFilter): HolidaySearchResult {
  ...
  if (filter.province) {
    filtered = filtered.filter(h => 
      !h.provinces || h.provinces.length === 0 || h.provinces.includes(filter.province!)
    )
  }

  return {
    holidays: filtered.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()),
 ...
 */
export function calculateLongWeekends(year: number, holidays: Holiday[]): LongWeekend[] {
 ...
 */
export function getProvinceName(province: Province): { id: string; en: string } {
  const provinceNames: Record<Province, { id: string; en: string }> = {
    ...
    'nusa_tenggara_barat': { id: 'Nusa Tenggara Barat', en: 'West Nusa Tenggara' },
    'nusa_tenggara_timur': { id: 'Nusa Tenggara Timur', en: 'East Nusa Tenggara' },
